...
CMmpGLIF* CMmpGLIF::CreateObject(HWND hWnd, HDC hDC)
{
    ...
    //pObj=new CMmpGL_MovieEx1(hWnd, hDC);
    if(pObj==NULL)
    {
        return pObj;
    }
...
#endif
{
	...
	//VV_SetMode_O( -500.0f, 500.0f, -500.0f, 500.0f, -500.0f,500.0f );
...
MMP_RESULT CMmpGL::Open()
{
	...
	// deselect it
	bFlag = wglMakeCurrent(m_hDC,m_hRC);
    MMPDEBUGMSG(1, (TEXT("[CMmpGL::Open()] wglMakeCurrent(m_hDC,m_hRC) %d "), bFlag ));

    bFlag = wglMakeCurrent(NULL,NULL);
    MMPDEBUGMSG(1, (TEXT("[CMmpGL::Open()] wglMakeCurrent(NULL, NULL) %d "), bFlag ));
...
int CMmpGL::TEX_UpdateRGB32(int imageWidth, int imageHeight, unsigned char* imageData, int n )
{
    //wglMakeCurrent(m_hDC, m_hRC);
    this->Draw_Begin();
...
#if 0
    glGenTextures(1, (GLuint*)&m_GLLIST_List[n]);
    glNewList( m_GLLIST_List[n], GL_COMPILE);
...
int CMmpGL::TEX_LoadRGB32(int imageWidth, int imageHeight, unsigned char* imageData, int n )
{
    ...
    //framebuffer setup
    glGenFramebuffersEXT( 1, (GLuint *)&m_GLLIST_List[10] );
    glBindFramebufferEXT( GL_FRAMEBUFFER_EXT, m_GLLIST_List[10] );
    glFramebufferTexture2DEXT( GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                           GL_TEXTURE_2D, m_GLLIST_List[n], 0 );
    //render target to default
