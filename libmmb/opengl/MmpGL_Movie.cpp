...
void CMmpGL_Movie::Draw_Objects()
{
    ...
    if( glIsList(m_GLLIST_List[1])==GL_TRUE )
    {
   //     glCallList(m_GLLIST_List[n]);

        glBindTexture(GL_TEXTURE_2D, m_GLLIST_List[1]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_imgWidth, m_imgHeight, GL_RGB, GL_UNSIGNED_BYTE, m_imgData);
    }
