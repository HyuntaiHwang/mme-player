...
/////////////////////////////////////////////////////////////
//CMmpEncoderVideo_FfmpegEx1 Member Functions

CMmpEncoderVideo_FfmpegEx1::CMmpEncoderVideo_FfmpegEx1(struct CMmpEncoderVideo::create_config *p_create_config) : 
...
CMmpEncoderVideo_FfmpegEx1::~CMmpEncoderVideo_FfmpegEx1()
{

}

MMP_RESULT CMmpEncoderVideo_FfmpegEx1::Open()
{
    ...
    if(mmpResult == MMP_SUCCESS) {
        mmpResult=CMmpEncoderFfmpegEx1::Open();
    }
    
    if(mmpResult == MMP_SUCCESS) {
        MMPDEBUGMSG(MMPZONE_MONITOR, (TEXT("[CMmpEncoderVideo_FfmpegEx1::Open] Success nForamt=(0x%08x %s) \n\r"), 
...
MMP_RESULT CMmpEncoderVideo_FfmpegEx1::Close()
{
    MMP_RESULT mmpResult;
    
    mmpResult=CMmpEncoderFfmpegEx1::Close();
    if(mmpResult!=MMP_SUCCESS)
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpEncoderVideo_FfmpegEx1::Close] CMmpEncoderFfmpeg::Close() \n\r")));
        return mmpResult;
    }
    ...
    if(mmpResult!=MMP_SUCCESS)
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpEncoderVideo_FfmpegEx1::Close] CMmpEncoderVideo::Close() \n\r")));
        return mmpResult;
    }
    
    MMPDEBUGMSG(MMPZONE_MONITOR, (TEXT("[CMmpEncoderVideo_FfmpegEx1::Close] Success nForamt=(0x%08x %s) \n\r"), 
                  this->get_fourcc_out(), this->get_fourcc_out_name() ));
...
MMP_RESULT CMmpEncoderVideo_FfmpegEx1::EncodeAu(class mmp_buffer_videoframe* p_buf_vf, class mmp_buffer_videostream* p_buf_vs) {
    ...
    if(m_pAVCodecContext == NULL) {
        CMmpEncoderFfmpegEx1::config enc_config;
        ...
        mmpResult = CMmpEncoderFfmpegEx1::EncodeDSI(&enc_config);
        if(mmpResult != MMP_SUCCESS) {
            return mmpResult;
        }
    }
