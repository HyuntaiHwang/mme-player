...
#define SOURCE_DEBUG 0 
#define AUDIO_DEC_DEBUG 0
#define VIDEO_DEC_DEBUG 0 
#define AUDIO_REN_DEBUG 0 
#define VIDEO_REN_DEBUG 0

/////////////////////////////////////////////////////////////
//CMmpPlayerAVEx4 Member Functions

CMmpPlayerAVEx4::CMmpPlayerAVEx4(CMmpPlayerCreateProp* pPlayerProp) : CMmpPlayer(pPlayerProp, "AVEx4" )
...
CMmpPlayerAVEx4::~CMmpPlayerAVEx4()
{
    
    
}

MMP_RESULT CMmpPlayerAVEx4::Open()
{
    ...
    if(mmpResult == MMP_SUCCESS ) {
        ...
        if(m_pDemuxer == NULL) {
            ...
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpPlayerAVEx4::Open] FAIL: this->CreateDemuxer")));
        }
        else {
...
MMP_RESULT CMmpPlayerAVEx4::Close()
{    
#if 0
    if(m_p_task_audio_render != NULL)
    {
...
MMP_RESULT CMmpPlayerAVEx4::PlayStop() {
...
void CMmpPlayerAVEx4::flush_queue_audio_stream()  {
...
void CMmpPlayerAVEx4::flush_queue_audio_frame() {
...
void CMmpPlayerAVEx4::flush_queue_video_stream() {
...
void CMmpPlayerAVEx4::flush_queue_video_frame() {
...
MMP_RESULT CMmpPlayerAVEx4::Seek(MMP_S64 pts) {
    ...
    MMPDEBUGMSG(1, (TEXT("[CMmpPlayerAVEx4::Seek] pts=%d "), (unsigned int)(pts/1000) ));
...
void CMmpPlayerAVEx4::Service_AudioDec() {
...
#if (CMmpPlayerAVEx4_AUDIO_DEC_TASK_ADD_PCM_ENABLE == 1)
                    m_p_mutex_audio_render->lock();
                    if(m_queue_audio_pcm.IsFull()) {
...
void CMmpPlayerAVEx4::Service_AudioRender() {
...
void CMmpPlayerAVEx4::Service_VideoDec() {
...
void CMmpPlayerAVEx4::Service_VideoRender() {

    MMP_S64 audio_pts, video_pts;
    MMP_TICKS diff;
    class mmp_buffer_videoframe *p_buf_vf;
    ...
    while(m_bServiceRun_VideoRender == MMP_TRUE) {
        ...
        while( (p_buf_vf != NULL)  && (m_bServiceRun_VideoRender == MMP_TRUE)) {
...
#if (CMmpPlayerAVEx4_DEMUX_TASK_ADD_AUDIO_DATA_ENABLE == 1)
            if(m_pDemuxer->GetAudioFourcc() != MMP_FOURCC_AUDIO_UNKNOWN) {

                if(m_pRendererAudio != NULL) {
                    audio_pts = m_pRendererAudio->get_real_render_pts();
                }
                else {
                    audio_pts = video_pts - (threshold+1000);
                }
            }
            else 
#endif
            {
                audio_pts = CMmpUtil::GetTickCountUS() - m_play_start_timestamp + m_seek_target_time;
            }
            

            diff =  (MMP_TICKS)(video_pts - audio_pts);
            m_mon_vren_last_diff = (MMP_S32)(diff/1000);

            if(diff <= threshold) { 
            
                //if( (audio_pts - video_pts) < 2000*1000 ) {
                if( (audio_pts - video_pts) < 500*1000 ) {
                    ...
                    MMPDEBUGMSG(VIDEO_REN_DEBUG, (TEXT("[AV Player VideoRen]  ts=%d / %d  (%d) Render, dur=%d "), 
                             (unsigned int)(video_pts/1000), 
                             (unsigned int)(audio_pts/1000),
                             (int)(diff/1000),
                             t2-t1,
                             m_queue_video_stream.GetSize(),
                             m_queue_video_yuv.GetSize() 
                             ));
                }
                else {
            ...
            else if( (audio_pts + 3*1000000LL) < video_pts ) {
                ...
                MMPDEBUGMSG(VIDEO_REN_DEBUG, (TEXT("[AV Player VideoRen]  ts=%d / %d  Wait(%d ms)"), 
                             (unsigned int)(video_pts/1000), 
                             (unsigned int)(audio_pts/1000),
                             (int)(diff/1000)
                             ));
...
#if (CMmpPlayerAVEx4_DEMUX_TASK_ADD_VIDEO_DATA_ENABLE == 1)
                    m_p_mutex_video_dec->lock();
