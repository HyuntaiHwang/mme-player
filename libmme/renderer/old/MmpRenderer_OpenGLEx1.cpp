...
CMmpRenderer_OpenGLEx1::CMmpRenderer_OpenGLEx1(struct CMmpRendererVideo::create_config* p_create_config) :  CMmpRendererVideo(p_create_config)
...
MMP_RESULT CMmpRenderer_OpenGLEx1::Init_OpenGL(MMP_S32 pic_width, MMP_S32 pic_height)
{
    if(m_pMmpGL)
    {
        m_pMmpGL->Close();
...
MMP_RESULT CMmpRenderer_OpenGLEx1::vf_config_internal(MMP_S32 bufcnt, MMP_S32 pic_width, MMP_S32 pic_height) {

    MMP_RESULT mmpResult;
    mmpResult = CMmpRendererVideo::vf_config_internal(bufcnt, pic_width, pic_height);
    if(mmpResult == MMP_SUCCESS) {
...
MMP_RESULT CMmpRenderer_OpenGLEx1::Render(class mmp_buffer_videoframe* p_buf_vf, MMP_MEDIA_ID decoder_id) {
    ...
    if(m_pMmpGL != NULL) {
        ...
        if(m_yuv_dump_hdl == NULL) {
            m_yuv_dump_hdl = yuv_dump_write_create("MmeVideRenOpenGL_Dump", pic_width, pic_height, CMmpRenderer_OpenGLEx1_DUMP_FOURCC);
        }
        if(m_dump_buffer == NULL) {
            ...
            case MMP_FOURCC_IMAGE_YUV420M: /* V4L2_PIX_FMT_YUV420M 12  YUV 4:2:0  3P non contiguous planes - Y, Cb, Cr */ 
...
#if (CMmpRenderer_OpenGLEx1_YUV_DUMP == 1)
                {
                    CMmpImageTool::ConvertYUV420MtoRGB(Y, U, V, 
                                                       ...
                                                       pic_width, pic_height,
                                                       m_dump_buffer, CMmpRenderer_OpenGLEx1_DUMP_FOURCC);
