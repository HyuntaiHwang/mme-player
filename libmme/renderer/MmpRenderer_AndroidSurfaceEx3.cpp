...
/////////////////////////////////////////////////////////////
//CMmpRenderer_AndroidSurfaceEx3 Member Functions

#define RENDER_FOURCC_IN MMP_FOURCC_IMAGE_YUV420
//#define RENDER_FOURCC_IN MMP_FOURCC_IMAGE_YVU420
//#define RENDER_FOURCC_IN MMP_FOURCC_IMAGE_ABGR8888

CMmpRenderer_AndroidSurfaceEx3::CMmpRenderer_AndroidSurfaceEx3(struct CMmpRendererVideo::create_config* p_create_config) :  
CMmpRendererVideo(p_create_config, RENDER_FOURCC_IN)
,m_queue_count(0)
,m_is_api_connect(false)
{
    

}

CMmpRenderer_AndroidSurfaceEx3::~CMmpRenderer_AndroidSurfaceEx3()
{

}

MMP_RESULT CMmpRenderer_AndroidSurfaceEx3::Open()
{
    ...
    MMPDEBUGMSG(MMPZONE_INFO, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::Open] DisplayInfo Resol(%d, %d) DPI(%3.1f %3.1f) density=%3.1f orientation=%d "), 
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        if(m_ComposerClient->initCheck() != NO_ERROR) {
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::Open] FAIL: mComposerClient = new SurfaceComposerClient")));
    ...
    /* Create Surface */
    if(mmpResult == MMP_SUCCESS) {
        //m_SurfaceControl = m_ComposerClient->createSurface(String8("CMmpRenderer_AndroidSurfaceEx3"), m_display_info.w, m_display_info.h, PIXEL_FORMAT_RGBA_8888, 0);
        m_SurfaceControl = m_ComposerClient->createSurface(String8("CMmpRenderer_AndroidSurfaceEx3"), m_display_info.w, m_display_info.h, PIXEL_FORMAT_BGRA_8888, 0);
        if(m_SurfaceControl == NULL) {
            mmpResult = MMP_FAILURE;
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::Open] FAIL: m_ComposerClient->createSurface")));
        }
        else { 
            if(m_SurfaceControl->isValid() != 1) {
                mmpResult = MMP_FAILURE;
                MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::Open] ERROR : m_SurfaceControl->isValid() ")));
            }
        }
    }
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        if(m_Surface == NULL) {
            mmpResult = MMP_FAILURE;
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::Open] FAIL: m_SurfaceControl->getSurface ")));
        }
        else {
...
MMP_RESULT CMmpRenderer_AndroidSurfaceEx3::Close()
{
    ...
    if(m_ComposerClient != NULL) {
        ...
        if(m_is_api_connect) {
            native_window_api_disconnect(m_NativeWindow.get(), NATIVE_WINDOW_API_MEDIA);
            m_is_api_connect = false;
        }

        native_window_set_buffers_transform(m_NativeWindow.get(), 0);
...
MMP_RESULT CMmpRenderer_AndroidSurfaceEx3::vf_config_internal(MMP_S32 bufcnt, MMP_S32 pic_width, MMP_S32 pic_height, struct mmp_rect display_crop_rect) {
...
MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::vf_config_internal] ln=%d "), __LINE__ ));                    
    ...
    native_window_api_connect(m_NativeWindow.get(), NATIVE_WINDOW_API_MEDIA);
    m_is_api_connect = true;
    ...
    native_window_set_buffers_dimensions(m_NativeWindow.get(),  pic_width, pic_height);
    native_window_set_buffers_format(m_NativeWindow.get(),  pixel_format);
...
MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::vf_config_internal] ln=%d  bufcnt=%d "), __LINE__, bufcnt ));                    
    for(i = 0; i < bufcnt; i++) {
            ...
            MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::vf_config_internal] ln=%d  bufcnt=%d err=%d"), __LINE__, bufcnt, err ));                    
            ...
            MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::vf_config_internal] ln=%d  bufcnt=%d "), __LINE__, bufcnt ));                    
               ...
               m_buf_native[i] = buf;
                ...
                MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_AndroidSurfaceEx3::vf_config_internal] buffer(%d/%d) pic(w:%d h:%d) d_rect(%d %d %d %d)  buf(w:%d h:%d stride:%d) ver:%d numfd:%d numInts:%d data:0x%08x fd:%d int(%d %d %d,,,) "), 
                        i, m_buf_count, 
...
MMP_RESULT CMmpRenderer_AndroidSurfaceEx3::Render(class mmp_buffer_videoframe* p_buf_vf, MMP_MEDIA_ID decoder_id) {
        ...
        if(fd1 == fd2) {
            ...
            if(m_queue_count > 0 ) {
                ...
                if( (err==0) && (buf != NULL) ) {
                    ...
                    /* clear display flag */
                    for(int ivf = 0; ivf < this->vf_get_count(); ivf++) {
                          class mmp_buffer_videoframe* p_vf_tmp = this->vf_get(ivf);
                          if(p_vf_tmp!=NULL) {
                              if(p_vf_tmp->get_buf_shared_fd() == fd3) {
                                    ((CMmpDecoderVideo*)decoder_id)->clear_display_flag(p_vf_tmp);
                                    MMPDEBUGMSG(RENDER_DEBUG, (TEXT("[AKDEBUG %s] ln=%d Clear Display Flag "), __func__, __LINE__ ));
                                    break;
                              }
                          }
                    }
