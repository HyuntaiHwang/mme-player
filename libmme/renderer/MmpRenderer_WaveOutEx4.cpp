...
/////////////////////////////////////////////////////////////
//CMmpRenderer_WaveOutEx4 Member Functions


void CALLBACK CMmpRenderer_WaveOutEx4::waveOutProcStub(HWAVEOUT WaveOutHandle, UINT uMsg, long dwInstance, DWORD dwParam1, DWORD dwParam2 ) 
{
    CMmpRenderer_WaveOutEx4* pObj=(CMmpRenderer_WaveOutEx4*)dwInstance;
    pObj->waveOutProc(WaveOutHandle, uMsg, dwInstance, dwParam1, dwParam2 );
}

CMmpRenderer_WaveOutEx4::CMmpRenderer_WaveOutEx4(CMmpRendererAudio::create_config *p_create_config) :  CMmpRendererAudio(p_create_config)
...
CMmpRenderer_WaveOutEx4::~CMmpRenderer_WaveOutEx4()
{

}

MMP_RESULT CMmpRenderer_WaveOutEx4::Open()
{
    ...
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpRenderer_WaveOutEx4::Open] FAIL: waveOutOpen(QUERY) \n\r")));
		m_WaveOutHandle = 0;
		return MMP_FAILURE;
	}

	mmresult = waveOutOpen(&m_WaveOutHandle, WAVE_MAPPER, &waveFormat, (DWORD)(VOID*)CMmpRenderer_WaveOutEx4::waveOutProcStub, (DWORD_PTR)this, CALLBACK_FUNCTION);
	if ( mmresult != MMSYSERR_NOERROR ) 
    {
		m_WaveOutHandle = 0;
		return MMP_FAILURE;
    ...
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpRenderer_WaveOutEx4::Open] FAIL: new CMmpWaveBuffer[CMmpWaveBuffer::WAVE_BUF_COUNT] \n\r")));
		return MMP_FAILURE;
	}
...
MMP_RESULT CMmpRenderer_WaveOutEx4::Close()
{
    MMP_RESULT mmpResult;
    MMP_S32 i;
    CMmpWaveBuffer* pWaveBuffer;
...
void CMmpRenderer_WaveOutEx4::waveOutProc(HWAVEOUT WaveOutHandle, UINT uMsg, long dwInstance, DWORD dwParam1, DWORD dwParam2 ) 
{
    WAVEHDR* pWaveHdr;
	...
	{ 
		...
		{ 			
            ...
            MMPDEBUGMSG(1, (TEXT("[WIM_DONE] id=%d pts=%d \n\r"), pMmpWaveBuffer->id, (MMP_U32)(pts/1000) ));
			break;
...
CMmpRenderer_WaveOutEx4::CMmpWaveBuffer* CMmpRenderer_WaveOutEx4::GetNextWaveBufferWithLock() {
    ...
    MMPDEBUGMSG(0, (TEXT("[CMmpRenderer_WaveOutEx4::GetNextWaveBufferWithLock] ++++++++++ \n\r")));
    ...
    while(1) {
        ...
        if(ret_idx  == -1) {
            MMPDEBUGMSG(0, (TEXT("[CMmpRenderer_WaveOutEx4::GetNextWaveBufferWithLock] Wait..")));
    ...
    MMPDEBUGMSG(0, (TEXT("[CMmpRenderer_WaveOutEx4::GetNextWaveBufferWithLock] ---------- id=%d buf=%d/%d free_buf_count=%d \n\r"), 
                   pWaveBuffer->id, 
...
void CMmpRenderer_WaveOutEx4::end_of_stream() {
    ...
    if(1) {
        ...
        //pWaveBuffer->wave_hdr.lpData =(LPSTR)pWaveBuffer->p_buf;
        pWaveBuffer->fill_size = pWaveBuffer->alloc_size;
        ...
        MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_WaveOutEx4::Render] waveOut fill=%d pts=%lld dwFlag=%d "),  
                  pWaveBuffer->fill_size, pWaveBuffer->pts,
...
MMP_RESULT CMmpRenderer_WaveOutEx4::Render(class mmp_buffer_audioframe* p_buf_af) {
    ...
    MMP_S32 wavebuf_remain_byte, copy_byte;
    ...
    if(wavebuf_remain_byte > src_size) {
        ...
        p_buf_dest = &pWaveBuffer->p_buf[pWaveBuffer->fill_size];
        memcpy(p_buf_dest, (void*)p_buf_src, wavebuf_remain_byte);
        pWaveBuffer->fill_size += wavebuf_remain_byte;
        copy_byte = wavebuf_remain_byte;

        is_send = MMP_TRUE;
    }
        
    pWaveBuffer->pts = p_buf_af->get_pts();
    if(is_send == MMP_TRUE) {
        pWaveBuffer->free_tick = CMmpWaveBuffer::BUSY_TICK;
        //pts_offset = 0;//mmp_audio_tool::pcm_cal_dur_us(pWaveBuffer->fill_size, sr, ch, bps);
        //pWaveBuffer->pts = p_buf_af->get_pts() + pts_offset;
        ...
        MMPDEBUGMSG(1, (TEXT("[CMmpRenderer_WaveOutEx4::Render] waveOut fill=%d src_remai=%d  Loop=%d dwFlag=%d "),  
                  pWaveBuffer->fill_size, src_size,
                  pWaveBuffer->wave_hdr.dwLoops,
                  pWaveBuffer->wave_hdr.dwFlags
                  ));
        ...
        pWaveBuffer = this->GetNextWaveBufferWithLock();
        wavebuf_remain_byte = pWaveBuffer->alloc_size - pWaveBuffer->fill_size;
        if(wavebuf_remain_byte > (src_size-copy_byte) ) {
            p_buf_dest = &pWaveBuffer->p_buf[pWaveBuffer->fill_size];
            if((src_size-copy_byte) > 0) {
                memcpy(p_buf_dest, (void*)&p_buf_src[copy_byte], (src_size-copy_byte) );
                pWaveBuffer->fill_size += (src_size-copy_byte);
            }
        }
