...
#include "MmpMuxer_FfmpegEx2.hpp"
#include "MmpUtil.hpp"
...
CMmpMuxer_FfmpegEx2::CMmpMuxer_FfmpegEx2(struct MmpMuxerCreateConfig* pCreateConfig) : CMmpMuxer(pCreateConfig)
,m_fp(NULL)
,m_iFileSize(0)
...
CMmpMuxer_FfmpegEx2::~CMmpMuxer_FfmpegEx2()
{

}

MMP_RESULT CMmpMuxer_FfmpegEx2::Open()
{
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        if(m_fp == NULL) {
            ...
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpMuxer_FfmpegEx2::Open] FAIL: fopen %s "), m_create_config.filename));
        }
    }
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        m_pAVIOContext = avio_alloc_context(m_IoBuffer, io_buffer_size, 1, m_Url, CMmpMuxer_FfmpegEx2::IORead_C_Stub, CMmpMuxer_FfmpegEx2::IOWrite_C_Stub, CMmpMuxer_FfmpegEx2::IOSeek_C_Stub);
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        if(m_oc == NULL) {
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpMuxer_FfmpegEx2::Open] FAIL: avformat_alloc_output_context2 ")));
            mmpResult = MMP_FAILURE;
            m_oc = NULL;
        }
        else {
      ...
      && (this->m_create_config.bMedia[MMP_MEDIATYPE_VIDEO] == MMP_TRUE) ) 
    {
        if(m_fmt->video_codec == AV_CODEC_ID_NONE) {
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpMuxer_FfmpegEx2::Open] FAIL: not support video codec (%c%c%c%c)"),
            ...
            if(m_video_st == NULL) {
                MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpMuxer_FfmpegEx2::Open] FAIL: add video stream")));
                mmpResult = MMP_FAILURE;
            }
        }
    }
...
MMP_RESULT CMmpMuxer_FfmpegEx2::Close()
{
...
MMP_RESULT CMmpMuxer_FfmpegEx2::AddMediaConfig(MMP_U32 mediatype, MMP_U8* buffer, MMP_U32 buf_size) {
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        if (ret < 0) {
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpMuxer_FfmpegEx2::AddMediaConfig] FAIL: avformat_write_header")));
            mmpResult = MMP_FAILURE;
        }
        else {
...
MMP_RESULT CMmpMuxer_FfmpegEx2::AddMediaData(MMP_U32 mediatype, MMP_U8* buffer, MMP_U32 buf_size, MMP_U32 flag, MMP_S64 pts) {
...
int CMmpMuxer_FfmpegEx2::IORead_C_Stub(void *opaque, uint8_t *buf, int buf_size) {

    URLContext *p_url = (URLContext*)opaque;
    CMmpMuxer_FfmpegEx2 *pObj = (CMmpMuxer_FfmpegEx2 *)p_url->priv_data;
    return pObj->IORead_C(opaque, buf, buf_size);
}

int CMmpMuxer_FfmpegEx2::IOWrite_C_Stub(void *opaque, uint8_t *buf, int buf_size) {
...
int64_t CMmpMuxer_FfmpegEx2::IOSeek_C_Stub(void *opaque, int64_t offset, int whence) {
...
int CMmpMuxer_FfmpegEx2::IORead_C(void *opaque, uint8_t *buf, int buf_size) {
...
int CMmpMuxer_FfmpegEx2::IOWrite_C(void *opaque, uint8_t *buf, int buf_size) {
...
int64_t CMmpMuxer_FfmpegEx2::IOSeek_C(void *opaque, int64_t offset, int whence) {
...
AVStream* CMmpMuxer_FfmpegEx2::add_stream(AVFormatContext *oc, AVCodec **codec, enum AVCodecID codec_id, enum MMP_MEDIATYPE mediatype)
{
