...
CMmpDecoderVideo_FfmpegEx1::CMmpDecoderVideo_FfmpegEx1(struct CMmpDecoderVideo::create_config *p_create_config) : 
...
CMmpDecoderVideo_FfmpegEx1::~CMmpDecoderVideo_FfmpegEx1()
{

}

MMP_RESULT CMmpDecoderVideo_FfmpegEx1::Open()
{
    ...
    //if(mmpResult == MMP_SUCCESS) {
    //    MMPDEBUGMSG(MMPZONE_MONITOR, (TEXT("[CMmpDecoderVideo_FfmpegEx1::Open] Success in=%s out=%s \n\r"),  this->get_fourcc_in_name(), this->get_fourcc_out_name() ));
    //}
...
MMP_RESULT CMmpDecoderVideo_FfmpegEx1::Close()
{
    MMP_RESULT mmpResult;
    
    mmpResult=CMmpDecoderFfmpegEx1::Close();
    if(mmpResult!=MMP_SUCCESS) {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderVideo_FfmpegEx1::Close] CMmpDecoderFfmpeg::Close() \n\r")));
        return mmpResult;
    }

    mmpResult=CMmpDecoderVideo::Close();
    if(mmpResult!=MMP_SUCCESS) {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderVideo_FfmpegEx1::Close] CMmpDecoderVideo::Close() \n\r")));
        return mmpResult;
    }
...
#if 1
int CMmpDecoderVideo_FfmpegEx1::ffmpeg_get_buffer_cb(struct AVCodecContext *p_context,  AVFrame *p_ff_pic) {
...
void CMmpDecoderVideo_FfmpegEx1::ffmpeg_release_buffer_cb(struct AVCodecContext *p_context,  AVFrame *p_ff_pic) {
...
MMP_RESULT CMmpDecoderVideo_FfmpegEx1::flush_buffer_in() {
...
MMP_RESULT CMmpDecoderVideo_FfmpegEx1::DecodeDSI(class mmp_buffer_videostream* p_buf_vs) {

    MMP_RESULT mmpResult;
    enum MMP_FOURCC fourcc_in, fourcc_out;

    mmpResult = CMmpDecoderFfmpegEx1::DecodeDSI(p_buf_vs, this->get_decoder_pic_width(), this->get_decoder_pic_height() );
    if(mmpResult == MMP_SUCCESS) {
...
MMP_RESULT CMmpDecoderVideo_FfmpegEx1::DecodeAu(class mmp_buffer_videostream* p_buf_vs, class mmp_buffer_videoframe** pp_buf_vf, enum ACTION *next_action) {
    ...
    *next_action = ACTION_NONE;
...
#if 1 //(MMPDECODERVIDEO_H264_AVC_CHECK == 1)
    /* src: queue buf */
    if( (this->get_fourcc_in() == MMP_FOURCC_VIDEO_H264) && (CMmpDecoderFfmpegEx1::m_is_avc ==1) ) {
        ...
        if( (p_avc1[0]==0) &&
            ...
            mmpResult  = CMmpH264Parser::ConvertAvc1ToH264(p_avc1, vs_size,  CMmpDecoderFfmpegEx1::m_nal_length_size, m_stream_buf, (MMP_S32*)&vs_size_sub, MMP_FALSE);
            if(mmpResult != MMP_SUCCESS) {
    ...
    MMPDEBUGMSG(0, (TEXT("[CMmpDecoderVideo_FfmpegEx1::DecodeAu] %d au(%02x %02x %02x %02x %02x %02x %02x %02x ) usedbyte=%d ausz=(%d/%d) pts=%d codec_width=%d codec_height=%d framefinished=%d "),
    ...
    if(frameFinished != 0) {
        ...
        if(p_buf_vf_decoded != NULL)     {
            ...
            MMPDEBUGMSG(1, (TEXT("[CMmpDecoderVideo_FfmpegEx1::DecodeAu] FAIL to this->get_next_vf(); ")));
        }
    }
...
MMP_RESULT CMmpDecoderVideo_FfmpegEx1::DecodeAu(class mmp_buffer_videostream* p_buf_vs, class mmp_buffer_videoframe* p_buf_vf) {
