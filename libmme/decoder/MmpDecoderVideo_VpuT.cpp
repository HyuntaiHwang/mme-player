...
#include "MmpDecoderVideo_VpuT.hpp"
#include "MmpUtil.hpp"
#include <ion.h>
...
/////////////////////////////////////////////////////////////
//CMmpDecoderVideo_VpuT Member Functions

MMP_BOOL CMmpDecoderVideo_VpuT::CheckSupportCodec(enum MMP_FOURCC fourcc_in) {
...
CMmpDecoderVideo_VpuT::CMmpDecoderVideo_VpuT(struct CMmpDecoderVideo::create_config *p_create_config) : 
...
,m_run_task_dec(MMP_FALSE)
,m_is_task_main_wait(MMP_FALSE)
,m_is_task_dec_wait(MMP_FALSE)
,m_p_task_dec(NULL)
,m_p_cond_dec_run(NULL)
,m_p_cond_dec_done(NULL)
,m_p_mutex_dec_run(NULL)
,m_p_mutex_dec_done(NULL)
...
CMmpDecoderVideo_VpuT::~CMmpDecoderVideo_VpuT()
{

}

MMP_RESULT CMmpDecoderVideo_VpuT::Open()
{
    ...
    m_p_cond_dec_run = mmp_oal_cond::create_object();
    m_p_cond_dec_done = mmp_oal_cond::create_object();
    m_p_mutex_dec_run = mmp_oal_mutex::create_object();
    m_p_mutex_dec_done = mmp_oal_mutex::create_object();

    m_run_task_dec = MMP_TRUE;
    m_p_task_dec = mmp_oal_task::create_object(CMmpDecoderVideo_VpuT::serviec_dec_stub, this);
    if(m_p_task_dec == NULL) {
...
MMP_RESULT CMmpDecoderVideo_VpuT::Close()
{
    MMP_RESULT mmpResult;

    if(m_p_task_dec != NULL) {
        m_run_task_dec = MMP_FALSE;
        m_p_cond_dec_run->signal(); //signal
        mmp_oal_task::destroy_object(m_p_task_dec);
        m_p_task_dec=NULL;
    }
    
    if(m_p_mutex_dec_run != NULL) {
        mmp_oal_mutex::destroy_object(m_p_mutex_dec_run);
        m_p_mutex_dec_run = NULL;
    }
    
    if(m_p_mutex_dec_done != NULL) {
        mmp_oal_mutex::destroy_object(m_p_mutex_dec_done);
        m_p_mutex_dec_done = NULL;
    }
    
    if(m_p_cond_dec_run != NULL) {
        mmp_oal_cond::destroy_object(m_p_cond_dec_run);
        m_p_cond_dec_run = NULL;
    }
    
    if(m_p_cond_dec_done != NULL) {
        mmp_oal_cond::destroy_object(m_p_cond_dec_done);
        m_p_cond_dec_done = NULL;
    }
    ...
    if(mmpResult!=MMP_SUCCESS)
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderVideo_VpuT::Close] CMmpDecoderFfmpeg::Close() \n\r")));
        return mmpResult;
    }
    ...
    if(mmpResult!=MMP_SUCCESS)
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderVideo_VpuT::Close] CMmpDecoderVideo::Close() \n\r")));
        return mmpResult;
    }
...
MMP_RESULT CMmpDecoderVideo_VpuT::DecodeAu(class mmp_buffer_videostream* p_buf_vs, class mmp_buffer_videoframe** pp_buf_vf, MMP_BOOL *is_reconfig) {
...
MMP_RESULT CMmpDecoderVideo_VpuT::Play_Function_Tool(MMP_PLAY_FORMAT playformat, MMP_S64 curpos, MMP_S64 totalpos)
{
	MMP_RESULT mmpResult = MMP_SUCCESS; 

    MMPDEBUGMSG(0, (TEXT("[CMmpDecoderVideo_VpuT] Play_Function_Tool!!!")));
...
MMP_RESULT CMmpDecoderVideo_VpuT::do_csc(class mmp_buffer_videoframe* p_buf_vf_vpu, class mmp_buffer_videoframe* p_buf_vf_decoded) {
...
#if 1
MMP_RESULT CMmpDecoderVideo_VpuT::do_csc_hw(class mmp_buffer_videoframe* p_buf_vf_vpu, class mmp_buffer_videoframe* p_buf_vf_decoded) {
...
MMP_RESULT CMmpDecoderVideo_VpuT::do_csc_sw(class mmp_buffer_videoframe* p_buf_vf_vpu, class mmp_buffer_videoframe* p_buf_vf_decoded) {
...
MMP_RESULT CMmpDecoderVideo_VpuT::do_csc_mscaler_test(class mmp_buffer_videoframe* p_buf_vf_vpu, class mmp_buffer_videoframe* p_buf_vf_decoded) {
...
MMP_RESULT CMmpDecoderVideo_VpuT::DecodeAu(class mmp_buffer_videostream* p_buf_vs, class mmp_buffer_videoframe* p_buf_vf) {
    ...
    /* Wait for dec-task ready */
    for(i = 0; i < 30; i++) {
        if(m_is_task_dec_wait == MMP_TRUE) {
            break;
        }
        CMmpUtil::Sleep(10);
    }
    if(m_is_task_dec_wait != MMP_TRUE) {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderVideo_VpuT::DecodeAu] ERROR: DecTask has some problem!! (Not Waitng) ")));
        return MMP_FAILURE;
    }

    /* Signal to Task, Dec Run */
    m_p_mutex_dec_run->lock();
    m_p_buf_vs_in = p_buf_vs;
    m_p_mutex_dec_run->unlock();

    m_is_task_main_wait = MMP_FALSE;
    m_p_cond_dec_run->signal();
    ...
    /*Wait Dec Done */
    m_p_mutex_dec_done->lock();
    m_is_task_main_wait = MMP_TRUE;
    m_p_cond_dec_done->wait(m_p_mutex_dec_done);
    m_p_mutex_dec_done->unlock();
...
void CMmpDecoderVideo_VpuT::serviec_dec_stub(void* parm) {

    CMmpDecoderVideo_VpuT* p_obj = (CMmpDecoderVideo_VpuT*)parm;
    p_obj->serviec_dec();
}
    
void CMmpDecoderVideo_VpuT::serviec_dec() {
    ...
    m_is_task_main_wait = MMP_FALSE;
    m_is_task_dec_wait = MMP_FALSE;
    while(m_run_task_dec) {

        /* Wait & Ready */
        m_p_mutex_dec_run->lock();
        m_is_task_dec_wait = MMP_TRUE;
        m_p_cond_dec_run->wait(m_p_mutex_dec_run);
        m_is_task_dec_wait = MMP_FALSE;
        p_buf_vs = m_p_buf_vs_in;
        m_p_buf_vs_in = NULL;
        m_p_mutex_dec_run->unlock();

        if(m_run_task_dec != MMP_TRUE) break;
        ...
        if(p_buf_vs != NULL) {
            dec_start_tick = CMmpUtil::GetTickCount();
            this->DecodeAu_PinEnd(p_buf_vs, &p_buf_vf_vpu);
            p_buf_vs->inc_stream_offset(p_buf_vs->get_stream_real_size());
            dec_end_tick = CMmpUtil::GetTickCount();
        }
        ...
        if(p_buf_vf_vpu != NULL) {
            ...
            m_p_buf_vf_vpu_last = p_buf_vf_vpu;
        }
        m_p_mutex_dec_done->unlock();

        for(i = 0; i < 1000; i++) {
            if(m_is_task_main_wait == MMP_TRUE) {
                break;
            }
            CMmpUtil::Sleep(1);
        }
        if(m_is_task_main_wait == MMP_TRUE) {
            m_p_cond_dec_done->signal();
        }
        else {
            MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderVideo_VpuT::serviec_dec] ERROR: Main Task has some problem!! (Not Waitng) ")));
        }
    }
}