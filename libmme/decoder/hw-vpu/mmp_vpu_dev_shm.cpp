...
MMP_RESULT mmp_vpu_dev_shm::open_vdi_memory() {
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        if(m_p_shm_vdi_obj->m_buf_addr_vdi_commmon.m_phy_addr != 0) {
            m_p_vpu_common_buffer = mmp_buffer_mgr::get_instance()->attach_dma_buffer(m_p_shm_vdi_obj->m_buf_addr_vdi_commmon);
        }
...
MMP_S32 mmp_vpu_dev_shm::streaming_ref_count() {

    MMP_S32 i, cnt = 0;
    class mmp_lock autolock(m_p_mutex);

    for(i = 0; i < VPU_MAX_NUM_INSTANCE; i++) {
        if(m_vpu_instance_hdl[i] != NULL) {
            cnt++;
        }
    }
...
MMP_BOOL mmp_vpu_dev_shm::streaming_is_full() {
...
MMP_RESULT  mmp_vpu_dev_shm::streaming_start(void *vpu_inst) {

    MMP_S32 i, ref_count;
    MMP_BOOL pm_on;
    class mmp_lock autolock(m_p_mutex);

#ifdef VPU_STREAMING_ENABLE

    MMP_RESULT mmpResult = MMP_SUCCESS;
    
    /* VPU_INST 를 Array 빈 공간에 넣는다. */
    pm_on = MMP_FALSE;
    for(i = 0; i < VPU_MAX_NUM_INSTANCE; i++) {
        if(m_vpu_instance_hdl[i] == NULL) {
            m_vpu_instance_hdl[i] = vpu_inst;
            pm_on = MMP_TRUE;
            break;
        }
    }

    /* PM-Domain을 켜야하면,  현재 ref-count 값을 참조해서 Driver에 요청한다. */
    if(pm_on == MMP_TRUE) {
        
        mmpResult = MMP_SUCCESS;
        ref_count = this->streaming_ref_count();
        
        if(ref_count == 1) {
            /* 실제 HW PM-Domain을 켠다 */
            mmpResult = mmp_vpu_hw::get_instance()->hw_ioctl(VDI_IOCTL_PM_DOMAIN_ON, NULL);
            if(mmpResult == MMP_SUCCESS) {
                RetCode vpu_ret;
                vpu_ret = ::VPU_Init(m_coreIdx);
                if( (vpu_ret == RETCODE_SUCCESS) ||  (vpu_ret == RETCODE_CALLED_BEFORE) ) {
                    ...
                    MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[mmp_vpu_dev_shm::streaming_start] FAIL: VPU_Init ")));
                    mmpResult = MMP_FAILURE;
                }
            }
        }
...
MMP_RESULT  mmp_vpu_dev_shm::streaming_stop(void *vpu_inst) {
    ...
    /* 입력된 instance와 array를 비교한다. 일치하면  array에서 삭제한다. */
    pm_off = MMP_FALSE;
    for(i = 0; i < VPU_MAX_NUM_INSTANCE; i++) {
        if(m_vpu_instance_hdl[i] == vpu_inst) {
            m_vpu_instance_hdl[i] = NULL;
            pm_off = MMP_TRUE;
            break;
        }
    ...
    if(pm_off == MMP_TRUE) {
        ref_count = this->streaming_ref_count();
        if(ref_count == 0) {
            /* 사용자가 없으므로, pm-domain을 끈다. */
            mmp_vpu_hw::get_instance()->hw_ioctl(VDI_IOCTL_PM_DOMAIN_OFF, NULL);
        }
    }
...
MMP_RESULT mmp_vpu_dev_shm::clk_enable() {

    class mmp_lock autolock(m_p_mutex);
    mmp_vpu_hw::get_instance()->hw_ioctl(VDI_IOCTL_CLK_ON, NULL);
    return MMP_SUCCESS;
}
	
MMP_RESULT mmp_vpu_dev_shm::clk_disable() {
    class mmp_lock autolock(m_p_mutex);
    mmp_vpu_hw::get_instance()->hw_ioctl(VDI_IOCTL_CLK_OFF, NULL);
    return MMP_SUCCESS;
}
