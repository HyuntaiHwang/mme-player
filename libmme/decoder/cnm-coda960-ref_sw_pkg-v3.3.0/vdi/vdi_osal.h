...
#define VLOG(level, fmt, args...) \
do { \
        if (level == ERR || level == TRACE) \
        { \
            ALOGE("[TID:%d-F:%s-L:%d] OMX-" fmt, gettid(), __func__, __LINE__, ##args); \
        } \
        else \
        { \
            ALOGV("[TID:%d-F:%s-L:%d] OMX-" fmt, gettid(), __func__, __LINE__, ##args); \
        } \
} while (0)
#else
