...
/////////////////////////////////////////////////////////////
//CMmpDecoderAudio_FfmpegEx1 Member Functions
...
CMmpDecoderAudio_FfmpegEx1::CMmpDecoderAudio_FfmpegEx1(struct CMmpDecoderAudio::create_config *p_create_config) : 
...
CMmpDecoderAudio_FfmpegEx1::~CMmpDecoderAudio_FfmpegEx1()
{

}

MMP_RESULT CMmpDecoderAudio_FfmpegEx1::Open() {
...
MMP_RESULT CMmpDecoderAudio_FfmpegEx1::Close()
{
    MMP_RESULT mmpResult;
    ...
    if(mmpResult!=MMP_SUCCESS)
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderAudio_FfmpegEx1::Close] CMmpDecoderFfmpeg::Close() \n\r")));
        return mmpResult;
    }
    ...
    if(mmpResult!=MMP_SUCCESS)
    {
        MMPDEBUGMSG(MMPZONE_ERROR, (TEXT("[CMmpDecoderAudio_FfmpegEx1::Close] CMmpDecoderAudio::Close() \n\r")));
        return mmpResult;
    }
...
MMP_RESULT CMmpDecoderAudio_FfmpegEx1::AudioConvert_Create(AVCodecContext *pAVCodecContext) {
...
MMP_RESULT CMmpDecoderAudio_FfmpegEx1::flush_buffer() {
...
MMP_RESULT CMmpDecoderAudio_FfmpegEx1::DecodeDSI(class mmp_buffer_audiostream *p_buf_as) {
...
MMP_RESULT CMmpDecoderAudio_FfmpegEx1::DecodeAu(class mmp_buffer_audiostream *p_buf_as, class mmp_buffer_audioframe *p_buf_af) {
    ...
    MMPDEBUGMSG(0, (TEXT("[CMmpDecoderAudio_FfmpegEx1::DecodeAu] sample_cnt=%d sz=%d (0x%02x 0x%02x 0x%02x 0x%02x ) "), m_sample_count++, avpkt.size,  avpkt.data[0], avpkt.data[1], avpkt.data[2], avpkt.data[3]  ));
    ...
    if(frameFinished != 0) {
        ...
        if( (m_pAVCodecContext->sample_rate != this->get_sample_rate()) 
            ...
            {
                MMPDEBUGMSG(1, (TEXT("[CMmpDecoderAudio_FfmpegEx1::DecodeAu] Reconfig Occur!!  (%d %d) => (%d %d) "),
    ...
    if(p_buf_af->get_coding_result() != mmp_buffer_media::RECONFIG) {
        ...
        if(!m_queue_decoded.IsEmpty()) {
            ...
            if((int)p_buf_af->get_buf_size() >= buf_remain_size) {
                ...
                MMPDEBUGMSG(1, (TEXT("[CMmpDecoderAudio_FfmpegEx1::DecodeAu] decoded buffer too small (%d %d"), p_buf_af->get_buf_size(), buf_remain_size));
            }
...
void CMmpDecoderAudio_FfmpegEx1::AudioConvert_Processing(MMP_TICKS uiTimeStamp, AVFrame *pAVFrame_Decoded, AVCodecContext *pAVCodecContext) {
        ...
        if(p_decoded_packet->p_buffer != NULL ) {
            ...
            if(m_pAudioConvert != NULL) {
                ...
                if(iret == 0) {
                    ...
                    if(this->get_channels() < m_pAVCodecContext->channels) {
                        ...
                        for(isam=0; isam < pAVFrame_Decoded->nb_samples; isam++) {
                            ...
                            sa_dest += this->get_channels();
                            sa_src += m_pAVCodecContext->channels;
                        }
                        p_decoded_packet->buf_size = pAVFrame_Decoded->nb_samples * this->get_channels() * this->get_bits_per_sample()/8; /* ch * sample_byte */
                    }
...
#else
void CMmpDecoderAudio_FfmpegEx1::PostProcessing(AVFrame *pAVFrame_Decoded, AVCodecContext *pAVCodecContext) {
